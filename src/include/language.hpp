...
typedef uint16_t LinePos;  // 0 based position of character on line
...
extern uint32_t LOGIC_ERROR_CNT;
...
struct TypeTag {
	...
	union {
		...
		struct {
			Type * index;
			Type * element;
			uint16_t step;					// Index will be multiplied by this value. Usually it is same as element size.
		};
...
//	int     value_nonempty;
	...
	Instr * src_i;
	Exp *   dep;
	uint16_t	read;			// how many times some instruction reads this variable (if 0, this is unused)
	uint16_t	write;			// how many times some instruction writes this variable (if 1 this is constant)

	uint16_t  set_index;		// index in current set
...
void HeapAddBlock(MemHeap * heap, uint32_t adr, uint32_t size);
void HeapRemoveBlock(MemHeap * heap, uint32_t adr, uint32_t size);
bool HeapAllocBlock(MemHeap * heap, uint32_t size, uint32_t * p_adr);
...
uint8_t IntByteSize(long * n);

Type * TypeAlloc(TypeVariant variant);
Type * TypeAllocConst(long * n);
Type * TypeAllocInt(long * min, long * max);
Type * TypeAllocIntN(int32_t min, int32_t max);
...
//uint16_t TypeItemCount(Type * type);
void TypeLimits(Type * type, Var ** p_min, Var ** p_max);

//void TypeLet(Type * type, Var * var);
typedef void (*RangeTransform)(long * dest, long * x, long * tr);
...
bool TypeIsN(Type * type, int32_t n);

long * TypeMax(Type * type);
long * TypeMin(Type * type);

Type * TypeUnion(Type * left, Type * right);

uint32_t TypeSize(Type * type);
uint32_t TypeAdrSize();
uint32_t TypeStructAssignOffsets(Type * type);
...
Var * FindOrAllocLabel(char * name, uint16_t idx);
...
void VarCount(Var * var, long * cnt);
void VarRange(Var * var, long ** p_min, long ** p_max);
...
void ProcUse(Var * proc, uint8_t flag);
...
VarTuple * VarSetItem(VarSet * set, uint16_t index);
...
struct LocTag {
	...
	InstrBlock * blk;
	Instr * i;
	uint32_t n;			// sequence number of instruction in block
};
...
struct InstrBlockTag {
	...
	uint32_t seq_no;				// Block sequence number. It is used to determine order of blocks when detecting loops.
...
void PrintProcFlags(Var * proc, uint32_t flags);
void PrintBlockHeader(InstrBlock * blk);
void PrintInstrLine(uint32_t n);
...
uint32_t InstrBlockInstrCount(InstrBlock * blk);
...
struct RuleArgTag {
	...
	uint8_t          arg_no;		// argument index (1..26)  used for variable & const
	union {
	   Var * var;
	   Type * type;
	   RuleArg * arr;
...
struct RuleTag {
	...
	uint8_t cycles;			// How many cycles the instruction uses.
};
...
bool InstrTranslate3(InstrOp op, Var * result, Var * arg1, Var * arg2, uint8_t mode);
...
struct BlockTag {
	...
	bool  negated;			// set to true, if the condition should be negated
};
...
typedef uint8_t * LiveSet;		// VarLive, VarDead, VarUndefined
...
int16_t VarTestReplace(Var ** p_var, Var * from, Var * to);
int16_t VarReplace(Var ** p_var, Var * from, Var * to);

int16_t InstrTestReplaceVar(Instr * i, Var * from, Var * to);
int16_t InstrReplaceVar(Instr * i, Var * from, Var * to);
...
uint8_t VarIsLiveInBlock(Var * proc, InstrBlock * block, Var * var);
